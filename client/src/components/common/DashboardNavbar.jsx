...
export default function DashboardNavbar() {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/88 backdrop-blur-xl border-b border-border">
        <div className="section-container h-16 flex items-center justify-between">
          <Link to={homePath} className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              <span className="font-heading text-lg font-bold text-primary block">Vriddhi</span>
              <span className="hidden md:block text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
                {user?.role === 'shop_owner'
                  ? 'Shop Workspace'
                  : user?.role === 'admin'
                    ? 'Admin Workspace'
          ...
          <div className="hidden md:flex items-center gap-1 rounded-full border border-border bg-white/75 px-2 py-1.5">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
          ...
          <div className="flex items-center gap-3">
            {user?.role === 'farmer' && (
              <Link to="/cart" className="relative p-2 rounded-xl hover:bg-secondary transition-colors">
                ...
                {cartCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 bg-accent text-accent-foreground text-[10px] rounded-full flex items-center justify-center font-bold">
                    {cartCount}
                  </span>
                )}
              </Link>
            ...
            <button onClick={handleLogout} className="hidden md:inline-flex btn-outline text-xs px-3 py-2">
              Logout
            </button>

            <button className="md:hidden text-foreground" onClick={() => setMenuOpen((open) => !open)}>
            ...
            <div className="section-container py-3 flex flex-col gap-1">
              ...
              <div className="border-t border-border pt-3 mt-2">
                ...
                <button
                  ...
                  className="w-full text-left px-3 py-2.5 text-sm text-destructive hover:bg-destructive/5 rounded-xl transition-colors"
                >
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-border z-40 flex justify-around py-2">
